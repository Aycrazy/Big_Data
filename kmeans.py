...
def initialize_centroid(data, ks):
    ...
    muks = {idx+1:data[k] for idx,k in enumerate(np.random.randint(0,data.shape[0],ks))}
...
def move_centroids(data,muks, ks,muk_assns):
    '''
    ...
    '''
    for k in muks:
        muks[k] = get_avg_features(data[muk_assns==k])
    return muks
...
def main(ks, cpus):
    ...
    while c < 10**4:
        ...
        if c:     
            ...
            temp = copy.deepcopy(muks)

            muks = move_centroids(cmf_array, muks, ks, muk_assns)
